...
class WPMLActions
{
    ...
    {
        global $wpdb;
        $data = $wpdb->get_results( "SELECT type, rank_id, subject, message, pushover_message FROM {$wpdb->prefix}uap_notifications;" );
        if ( !$data ){
            return;
        }
        $domain = 'uap';
    ...
    {
        global $wpdb;
        $data = $wpdb->get_results( "SELECT id, name, description FROM {$wpdb->prefix}uap_banners;" );
        if ( !$data ){
            return;
        }
        $domain = 'uap';
    ...
    {
        global $wpdb;
        $data = $wpdb->get_results( "SELECT id, label FROM {$wpdb->prefix}uap_ranks;" );
        if ( !$data ){
            return;
        }
        $domain = 'uap';
